...
def show_image_multi(df, image_path, width, height, trail_num, method, root1):
...
def extract_data(image_index):
    ...
    column_names = ['image_index num', 'eye fix', 'x start (pixels)', 'y start (pixels)', 'x end (pixels)',
                    ...
                    'a', 'amplitude in pixels', 'amplitude in degrees', 'write peak velocity deg/s',
                    'write average velocity deg/s', 'b', 'size', 'category', 'trail number', 'source_csv']
    ...
    for idx, csv_file in enumerate(csv_files):
        ...
        with open(file_path, 'r') as f:
            ...
            for row in reader:
                if row[0] == str(image_index):
                    # Add the 'source_csv' column with the CSV file's index
                    row.append(idx)

                    # Convert the row to a DataFrame and append it to the combined DataFrame
                    selected_row_df = pd.DataFrame([row], columns=column_names)
                    combined_df = pd.concat([combined_df, selected_row_df], ignore_index=True)
